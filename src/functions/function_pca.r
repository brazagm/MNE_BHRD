...
# date:     29-04-2018
# edited by: alan braz, 17-11-2022
# obs: @brazagm was responsible by updating the code for terra
...
#raster::rasterOptions(chunksize = 1e+10)
  ...
  # data var present
  da_pres <- terra::as.points(raster_pres) %>% 
  ...
  pca_pres_raster <- terra::rast(eix_pres_95_coords)
  ...
  # raster
  terra::writeRaster(x = pca_pres_raster, 
                      #filename = paste0(prefix_pca_raster_pres, "_pc0", seq(cum_var_95)),
                      filename = paste0(path_output_pca_pres, "/", prefix_pca_raster_pres, "_pc0", seq(cum_var_95),".asc"), 
     #                 bylayer = TRUE,
  ...
  # data var future
  da_proj <- terra::as.points(raster_proj) %>% 
    as.data.frame(., geom = "XY") %>%
    as_tibble() %>%
    na.omit() %>%
    relocate(x:y)
  ...
  pca_proj_raster <- terra::rast(eix_proj_95_coords)
  ...
  raster::writeRaster(x = pca_proj_raster, 
                      ...
                      filename = paste0(path_output_pca_proj, "/", prefix_pca_raster_proj, "_pc0", seq(cum_var_95), ".asc"), 
                      #bylayer = TRUE,
                      #options = c("COMPRESS=DEFLATE"), 
                      #format = "ascii", 
                      overwrite = TRUE)
